...
//prints stream before applying the flatMap() method
        ...
        allproducts.stream().flatMap(item -> item.stream().map(x -> x)).collect(Collectors.toList()).forEach(System.out::println);
        allproducts.stream().flatMap(pLis -> pLis.stream().map(item -> item + "Hello Again" )).collect(Collectors.toList());
